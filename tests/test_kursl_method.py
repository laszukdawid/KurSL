...
class TestKurslMethod(unittest.TestCase):
    ...
    def test_set_prior_default_setting(self):
        ...
        self.assertTrue(np.all(kursl.theta_init == params), "Same array is expected")
    ...
    def test_set_prior_incorrect_shape_update_theta(self):
        ...
        self.assertTrue(
            np.all(kursl.theta_init == params),
    ...
    def test_detrend_default(self):
        ...
        self.assertEqual(np.round(np.mean(S_default), 7), 0, "Default detrend is mean removing")

    def test_detrend_mean(self):
        S = np.random.random(100) * 2 - 1
        S_nomean = KurslMethod.detrend(S, remove_type="mean")
        self.assertEqual(np.round(np.mean(S_nomean), 7), 0, "Removing mean should return mean 0")

    def test_detrend_cubic(self):
        t = np.arange(0, 1, 0.005)
        S = t**3 + 2 * (t - 0.5) ** 2 + 5
        S_nocubic = KurslMethod.detrend(S, remove_type="cubic")
        self.assertEqual(np.round(np.mean(S_nocubic), 7), 0, "Removing mean should return mean 0")
    ...
    def test_cost_lnprob(self):
        ...
        self.assertTrue(
            ...
            "Expected likelihood ({} +- {}), but got {}".format(expected_like, eps, like),
        )
    ...
    @unittest.skipIf(QUICK_TEST, "Skipping in quick runs as it takes too long to execute.")
    def test_run_optimize_default(self):
        ...
        self.assertTrue(
            np.allclose(theta, _theta, rtol=1e-1, atol=1e-1),
    ...
    def test_run_custom_theta(self):
        ...
        self.assertEqual(theta.shape, theta_init.shape, "Shape shouldn't change")
        self.assertEqual(kursl.nH, nH)

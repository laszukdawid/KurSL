...
class TestMCMC(unittest.TestCase):
    ...
    def test_init_walkers_proper_scaling_with_K(self):
        ...
        self.assertTrue(
            np.allclose(theta_mcmc[0, 3:], [4.9, 4.9]),
            "If W==sum(K) all couplings are scaled by 0.98*W, same "
            "as for W<sum(K).\nReceived: {}\nExpected: {}".format(theta_mcmc[0, 3:], [4.9, 4.9]),
        )
        self.assertTrue(
            np.allclose(theta_mcmc[1, 3:], [5, 5]),
            "For W>sum(K) nothing should be changed." "\nReceived: {}\nExpected: {}".format(theta_mcmc[1, 3:], [5, 5]),
        )
        self.assertTrue(
            np.allclose(theta_mcmc[2, 3:], [2.8, 16.8]),
    ...
    def test_lnprob_theta_outside_range(self):
        ...
        self.assertEqual(
            KurslMCMC.lnprob(theta, t, S[:-1], model),
            -np.inf,

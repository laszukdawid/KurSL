...
class TestPreprocessor(unittest.TestCase):
    ...
    def test_remove_peak_norm(self):
        ...
        self.assertTrue(np.allclose(param_in, param_out), "Parameters should be the same")
        self.assertTrue(np.allclose(peak_in, peak_out), "Peaks should be the same")
    ...
    def test_determine_params_ptype(self):
        ...
        self.assertEqual(
            norm_params.shape,
            lorentz_params.shape,
            "Expecting same number of oscillators",
        )
        self.assertTrue(
            np.allclose(norm_params[:, 0], lorentz_params[:, 0], atol=1e-1),
            "Expecting similar frequencies,\n" + str(norm_params[:, 0] - lorentz_params[:, 0]),
        )
        self.assertTrue(
            np.allclose(norm_params[:, 1], lorentz_params[:, 1], atol=1e-1),
